...
/* ---------------------------- */
    ...
    if (!MMO_Init()) {
    	MessageBoxA(NULL, "Init failed.", "Error!", MB_OK);
    	return false;
    }
...
void VBA::loadSettings()
{
  ...
  systemVerbose = regQueryDwordValue("verbose", 0);
  systemDebug = regQueryDwordValue("debug", 0);
  ddrawDebug = regQueryDwordValue("ddrawDebug", false) ? true : false;
}
...
void VBA::saveSettings()
{
  ...
  regSetDwordValue("verbose", systemVerbose);
  regSetDwordValue("debug", systemDebug);
  regSetDwordValue("ddrawDebug", (ddrawDebug ? 1 : 0));
}
